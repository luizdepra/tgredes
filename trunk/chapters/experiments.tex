...
\section{Configurações e Ambiente de Experimentação} \label{config_experiments}
	...
	memória DDR 400 de 512MB e disco \textit{Samsung} 40GB, 7200RPM.
...
\section{Decisões de Implementação} \label{impl_decisions}
	...
	tomada empiricamente. Nesse experimento, comparamos a árvore \textit{K-D}
	...
	multidimensionais de 4 chaves. No MySQL, utilizamos somente arquivo de
	...
	experimentos foram feitos com cinco bases diferentes, sendo que o valores
	...
	ferramentas. Mesmo que a origem dos dados seja randômica, isso não afetaria
	o resultado em nenhum aspecto, visto que seriam dados de mesma natureza.
...
\section{Comparação da árvore \textit{K-D} com o MySQL} \label{kd_vs_mysql}
	...
	busca por intervalo. Esse intervalo é de 5, e o valor pelo qual varia é 50.
	...
	consiste em ir construindo a estrutura multidimensional na medida em que os
	dados são obtidos no monitoramento.
...
\label{kd_vs_mysql_insert}
	...
	MySQL, as chaves variam de 4, 8 a 16.
...
16.} \label{img_insert_Xkeys_kd_mysql} \end{center} \end{figure}
	...
	a inserção em 1 minuto e 43 segundos.
...
\label{img_search_Xkeys_mysql} \end{center} \end{figure}
	...
	seja, não apareceria nenhuma linha no gráfico de comparação. A figura
	\ref{img_search_Xkeys_mysql} mostra o tempo para a busca no MySQL, com
	chaves variando entre 4, 8 e 16. Observa-se que a partir de 2 milhões de
	registros e 16 chaves, o MySQL acentua a sua curva de crescimento em muito
	mais do que seria o padrão esperado.
...
\section{Comparação da árvore \textit{K-D-B} com o MySQL} \label{kdb_vs_mysql}
	...
	\cite{andreas}. A subseção \ref{kdb_vs_mysql_insert} discute os experimentos
	com a operação de inserção. Terminando com a subseção
...
%\label{kdb_vs_mysql2_insert}
	...
	MySQL, através da operação LOAD DATA INFILE \cite{mysqlLIF}.
...
\label{img_insert_10keys_kdb_mysql_2} \end{center} \end{figure}
	...
	2), inseção de registro (peso proporcional ao tamanho do registro), inseção
...
\label{img_search_10keys_kdb_mysql} \end{center} \end{figure}
	...
	índice de busca. Ao utilizar 10 chaves no índice, a busca o MySQL passa a
	...
	milhões de registros, isso ocorre porque nesses experimentos porque foram
