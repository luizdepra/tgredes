...
\section{Árvores \textit{K-D-B}} \label{kdb}
	...
	A árvore \textit{K-D-B} \cite{andreas} é utilizada principalmente para
...
\end{figure}
	...
	exato de pesquisa. Os passos desse algoritmo esão descritos a seguir:

	\begin{verbatim} (L1) Iniciando na página raiz, visitar a página; (L2) Se a
	página visitada é página folha, então retorna o ponto na página ou falso
	caso o ponto não esteja na página; (L3) Senão, enquanto a página visitada
	for página de região faça: recupera a página visitada da memória, localiza a
		página descendente que intercepta o ponto, faz essa a página a ser
		visitada e vai para (L2); \end{verbatim}
	...
	\begin{verbatim} (L1) Iniciando na página raiz, visitar a página; (L2) Se a
	página visitada é página folha, então retorna todos os pontos na página que
	pertencem ao intervalo especificado; (L3) Senão, enquanto a página visitada
	for página de região faça: recupera a página visitada da memória, para cada
		página que intercepta o intervalo especificado, e caso não seja vazio,
		faz essa a página a ser visitada e vai para (L2); \end{verbatim}
...
\caption{Exemplo de árvore \textit{B+}.} \end{center} \end{figure}
	...
	estão entre elas o \textit{MyISAM} (padrão do MySQL) ambas possibilitam a
	criação de índices com múltiplas chaves (até 15 chaves).
